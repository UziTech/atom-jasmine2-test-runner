...
export default class TestFailures {
	constructor(props) {
		...
		this.itemsToRender = new Set();// TODO:
	...
	render() {
		return (
			<div className='failures-container'>
				{/* this.props.rootSuite.suites.map(this.renderTopLevelSuite.bind(this)) // TODO: */}
			</div>
		);
	}
	...
	renderTopLevelSuite(suite) {
		if (this.itemsToRender.has(suite)) {
			return (
				<Collapser className='root' header={< span className = 'header' > {
					suite.title
				} < /span>}>
	...
	renderSubSuite(suite) {
		if (this.itemsToRender.has(suite)) {
			return (
				<Collapser className='tests-container' header={< span > {
					suite.title
				} < /span>}>
					<div className='tests-container'>
						{this.renderSuiteFailures(suite)}
