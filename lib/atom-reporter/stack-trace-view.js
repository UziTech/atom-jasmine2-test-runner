			...
			if (this.props.specDirectory) {
				...
				if (match) {
					...
					line = (
						<span>
							{lineSegments[0]}
							<a href={`atom://core/open/file?filename=${encodeURIComponent(match[1])}&line=${match[3]}${match[4] ? `&col=${match[5]}` : ""}`} on={{ click: this.handleFileLinkClick }} title="Open in Atom">{`${match[2]}:${match[3]}${match[4]}`}</a>
							{lineSegments[1]}
						</span>
					);
				}
