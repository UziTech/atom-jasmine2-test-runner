...
`;
	...
	getSpecs(suite) {
		...
		for (let child of suite.children) {
			if (child instanceof jasmine.Spec) {
				child.suite = suite;
				child.specType = "user"; // TODO:
				child.specDirectory = ""; // TODO:
				specs.push(child);
			} else {
	...
	addSpecs() {
		...
		this.specs = this.getSpecs(jasmine.getEnv().topSuit());
		let coreSpecs = 0;
		...
		if (userPackageSpecs > 0) {
			if ((coreSpecs === 0) && (bundledPackageSpecs === 0)) {
				...
				let { specDirectory } = this.specs[0];
				let packageFolderName = path.basename(path.dirname(specDirectory));
