...
export default class AtomReporter {
	...
	constructor(options) {
		...
		this.specDirectory = options.specDirectory;
		...
		this.tabs.innerHTML = `
<span id="j2-tab" class="tab not-available">Jasmine 2.x</span>
<span id="j1-tab" class="tab not-available">Jasmine 1.x</span>
`;
...
`;
	...
	jasmineStarted(suiteInfo) {
		...
		if (this.totalSpecCount > 0) {
			document.querySelector("#j2-tab").classList.remove("not-available");
	...
	handleEvents() {
		...
		listen(document, "click", ".tab", event => {
			if (event.currentTarget.classList.contains("active") || event.currentTarget.classList.contains("not-available")) {
				return;
			}
			if (event.currentTarget.id === "j2-tab") {
				document.querySelector(".j2-spec-reporter-container").classList.remove("not-active");
				document.querySelector(".spec-reporter-container").classList.add("not-active");
			} else {
				document.querySelector(".spec-reporter-container").classList.remove("not-active");
				document.querySelector(".j2-spec-reporter-container").classList.add("not-active");
			}
	...
	getSpecs(suite) {
		...
		for (let child of suite.children) {
			if (child instanceof jasmine.Spec) {
				child.suite = suite;
				child.specType = "user";
				child.specDirectory = this.specDirectory; // TODO:
				specs.push(child);
			} else {
	...
	addSpecs() {
		...
		if (userPackageSpecs > 0) {
			if ((coreSpecs === 0) && (bundledPackageSpecs === 0)) {
				...
				let packageFolderName = path.basename(path.dirname(this.specDirectory));
				let packageName = _.undasherize(_.uncamelcase(packageFolderName));
...
`;
		...
		for (let result of this.spec.result.failedExpectations) {
			let stackTrace = this.formatStackTrace(this.spec, result.message, result.stack);
	...
	formatStackTrace(spec, message, stackTrace) {
		...
		let jasminePattern = /^\s*at\s+.*\(?.*[/\\]jasmine(-[^/\\]*)?\.js:\d+:\d+\)?\s*$/;
		let firstJasmineLinePattern = /^\s*at [/\\].*[/\\]jasmine(-[^/\\]*)?\.js:\d+:\d+\)?\s*$/;
		let lines = [];
		...
		for (let index = 0; index < lines.length; index++) {
			...
			let prefixMatch = line.match(/at \.<anonymous> \(([^)]+)\)/);
